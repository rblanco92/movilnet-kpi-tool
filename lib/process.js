...
// onProgress(n) se llama periódicamente con el número de filas leídas.
function streamSheet(zipU8, sst, onRow, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    if (!started) reject(new Error("No se encontró la hoja de datos en el archivo."));
  });
}
...
export async function processLTE(file, startStr, endStr, onProgress) {
  ...
  await streamSheet(
    ...
    (row) => {
      if (header === null) {
        header = row.map((x) => (x == null ? "" : String(x).trim()));
        cStart = colIndex(header, "Start Time");
        cCell = colIndex(header, "Cell");
        if (cStart < 0) throw new Error('No se encontró la columna "Start Time".');
        if (cCell < 0) throw new Error('No se encontró la columna "Cell".');
        metricCols = LTE_METRICS.map(([label, hname]) => {
          const idx = colIndex(header, hname);
          if (idx < 0) throw new Error(`No se encontró la columna "${hname}".`);
          return { label, idx };
        });
        return;
      }
    ...
    (n) => onProgress && onProgress(`Procesando… ${n.toLocaleString()} filas`)
  );
  ...
  ws.addRow([
    ...
    "Local Cell ID",
    ...metricCols.map((m) => m.label),
  ]);
  for (const key of keys) {
    const g = groups.get(key);
    const rowVals = [g.enb, g.lcid === "" ? "" : parseInt(g.lcid) || g.lcid];
    for (let i = 0; i < metricCols.length; i++) rowVals.push(avg(g.metrics[i]));
    ws.addRow(rowVals);
  }
...
/* ============================================================
   PRESET UMTS (comparativa Antes / Despues)
   ============================================================ */
const SMAP = { 1: 1, 4: 1, 2: 2, 5: 2, 3: 3, 6: 3 };

const UMTS_METRICS = [
  { label: "Disponibilidad UMTS (%)", header: "Disponibilidad UMTS (%)", part: 1 },
  { label: "CS_TRAFFIC_UMTS (Erl)", header: "CS_TRAFFIC_UMTS (Erl)", part: 1 },
  { label: "TraficoPS (MB)", header: "TraficoPS (MB)", part: 1 },
  { label: "U_HSDPA.UE.Mean.Cell", header: "U_VS.HSDPA.UE.Mean.Cell (None)", part: 1 },
  { label: "CS_ServiceDropRatio (%)", header: "CS_ServiceDropRatio (%)", part: 1 },
  { label: "PS_CallDropRatio_OptRF (%)", header: "PS_CallDropRatio_OptRF (%)", part: 1 },
  { label: "Retencion Datos (%)", header: "Retencion Datos (%)", part: 1 },
  { label: "Retencion Voz (%)", header: "Retencion Voz (%)", part: 1 },
  { label: "Accesibilidad Voz (%)", header: "Accesibilidad Voz (%) - LB (%)", part: 2 },
  { label: "Accesibilidad Datos (%)", header: "Accesibilidad Datos (%) - LB (%)", part: 2 },
];
...
export async function processUMTS(files, antes, despues, onProgress) {
  if (!files || files.length < 2)
    throw new Error("Se necesitan los dos archivos: UMTS Parte 1 y Parte 2.");
    ...
    await streamSheet(
      ...
      (row) => {
        if (header === null) {
          ...
          partsSeen.add(part);
          cStart = colIndex(header, "Start Time");
          cCell = colIndex(header, "cellid");
          cSector = colIndex(header, "sector");
          if (cStart < 0 || cCell < 0 || cSector < 0)
            throw new Error(
              `Faltan columnas clave (Start Time / cellid / sector) en "${file.name}".`
            );
        ...
        let cellid = row[cCell];
        if (typeof cellid === "number") cellid = Math.trunc(cellid);
        const sec = row[cSector];
        const secInt = typeof sec === "number" ? Math.trunc(sec) : parseInt(sec);
        const msec = SMAP[secInt] != null ? SMAP[secInt] : secInt;
  ...
  if (!partsSeen.has(1) || !partsSeen.has(2))
    throw new Error(
      "Debes subir ambos archivos: uno debe ser Parte 1 y otro Parte 2."
    );
  if (data.size === 0)
  ...
  for (const key of keys) {
    ...
    for (const m of UMTS_METRICS) {
      rowVals.push(avg(g.antes[m.label]));
      rowVals.push(avg(g.despues[m.label]));
    }
